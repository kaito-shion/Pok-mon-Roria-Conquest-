...
local gateOptions = {}
local currentGateHighlight = nil
...
local pokemonCenterFound = false
local pokemonCenterLabel = nil
...
-- ================================================
-- MAIN TAB - GATE TELEPORT
-- ================================================
MainTab:CreateLabel("🚪 Gates")
	...
	for _, chunk in pairs(workspace:GetChildren()) do
		...
		if string.match(lowerName, "^chunk%d+$") then
			for _, v in pairs(chunk:GetChildren()) do
				if v.Name == "Gate" and v:IsA("Model") then
					table.insert(gateOptions, v:GetFullName())
					print('[GATE] Found: "' .. v:GetFullName() .. '"')
				end
			end
		end
	end
...
-- Gate dropdown
local GateDropdown = MainTab:CreateDropdown({
	Name = "🚪 Select Gate",
	Options = gateOptions,
	CurrentOption = nil,
	Callback = function(option)
		option = normalizeOption(option)
		selectedGate = option
		print('[GATE] Selected: "' .. tostring(option) .. '"')
		
		-- Remove previous highlight
		removeHighlight(currentGateHighlight)
		currentGateHighlight = nil
		
		-- Add highlight to selected gate
		if option ~= "NO GATES FOUND" then
			for _, chunk in pairs(workspace:GetChildren()) do
				local lowerName = string.lower(chunk.Name)
				if string.match(lowerName, "^chunk%d+$") then
					local gate = chunk:FindFirstChild("Gate")
					if gate and gate:IsA("Model") then
						currentGateHighlight = addHighlight(gate, Color3.fromRGB(0, 191, 255))
						print('[GATE] Highlighted: "' .. option .. '"')
						break
					end
				end
			end
...
-- Refresh gate list button
MainTab:CreateButton({
	Name = "🔄 Refresh Gate List",
	Callback = function()
		-- Remove highlight when refreshing
		removeHighlight(currentGateHighlight)
		currentGateHighlight = nil
		...
		getGates()
		GateDropdown:Refresh(gateOptions, true)
		print("[GATE] Gate list refreshed")
	end
})

-- Teleport to gate function
local function teleportToGate(model)
	local char = player.Character or player.CharacterAdded:Wait()
	local root = char:FindFirstChild("HumanoidRootPart")
	
	if not root then
		print("[GATE] No HumanoidRootPart found on player")
		return
	end
...
-- Teleport to gate button
MainTab:CreateButton({
	Name = "🚀 Teleport to Gate",
	Callback = function()
		if not selectedGate or selectedGate == "NO GATES FOUND" then
			print("[GATE] Invalid selection")
			return
		end
		...
		for _, chunk in pairs(workspace:GetChildren()) do
			local lowerName = string.lower(chunk.Name)
			if string.match(lowerName, "^chunk%d+$") then
				gate = chunk:FindFirstChild("Gate")
				if gate then break end
			end
		end
		
		if not gate then
			print("[GATE] Gate not found")
			return
		end
		
		print('[GATE] Teleporting to Gate')
		teleportToGate(gate)
	end
})
...
-- ================================================
-- MAIN TAB - POKEMON CENTER
-- ================================================
MainTab:CreateLabel("🏥 Pokemon Center")

-- Check for Pokemon Center
local function checkPokemonCenter()
	pokemonCenterFound = false
	
	for _, chunk in pairs(workspace:GetChildren()) do
		local lowerName = string.lower(chunk.Name)
		if string.match(lowerName, "^chunk%d+$") then
			local pokemonCenter = chunk:FindFirstChild("Pokemon Center")
			if pokemonCenter and pokemonCenter:IsA("Model") then
				pokemonCenterFound = true
				print("[POKEMON CENTER] Found in " .. chunk.Name)
				break
			end
		end
	end
	
	if pokemonCenterLabel then
		if pokemonCenterFound then
			pokemonCenterLabel:Set("Pokemon Center - ✅ FOUND")
		else
			pokemonCenterLabel:Set("Pokemon Center - ❌ NOT FOUND")
		end
	end
	
	return pokemonCenterFound
end

-- Create Pokemon Center status label
pokemonCenterLabel = MainTab:CreateLabel("Pokemon Center - Checking...")
checkPokemonCenter()

-- Refresh Pokemon Center button
MainTab:CreateButton({
	Name = "🔄 Refresh Pokemon Center",
	Callback = function()
		checkPokemonCenter()
		print("[POKEMON CENTER] Refreshed")
	end
})

-- Teleport to Pokemon Center function
local function teleportToPokemonCenter()
	local char = player.Character or player.CharacterAdded:Wait()
	local root = char:FindFirstChild("HumanoidRootPart")
	
	if not root then
		print("[POKEMON CENTER] No HumanoidRootPart found on player")
		return
	end
	...
	for _, chunk in pairs(workspace:GetChildren()) do
		...
		if string.match(lowerName, "^chunk%d+$") then
			local pokemonCenter = chunk:FindFirstChild("Pokemon Center")
			if pokemonCenter and pokemonCenter:IsA("Model") then
				if pokemonCenter.PrimaryPart then
					root.CFrame = pokemonCenter.PrimaryPart.CFrame * CFrame.new(0, 0, -10)
				else
					local part = pokemonCenter:FindFirstChildWhichIsA("BasePart", true)
					if part then
						root.CFrame = part.CFrame * CFrame.new(0, 0, -10)
					end
				end
				print("[POKEMON CENTER] Teleported")
				return
			end
		end
	end
	
	print("[POKEMON CENTER] Not found")
end

-- Teleport to Pokemon Center button
MainTab:CreateButton({
	Name = "🚀 Teleport to Pokemon Center",
	Callback = function()
		if not pokemonCenterFound then
			print("[POKEMON CENTER] Pokemon Center not found. Please refresh.")
			return
		end
		teleportToPokemonCenter()
	end
})
